...
import { PokerStore, TournamentRow, TournamentPreset, TournamentConfig } from '@/types/poker';
...
function generateBlindsGrid(config: TournamentConfig): { tempGrid: TournamentRow[]; totalMinutes: number } {
  let totalMinutes = 0;
  ...
  while (gameLevelCounter <= 20) {
    ...
    tempGrid.push({ isBreak: false, levelNum: gameLevelCounter, labelText: `Уровень ${gameLevelCounter}`, sb, bb: currentBB, ante, duration: config.levelDuration });
    totalMinutes += config.levelDuration;
    blindsPointer++;

    if (gameLevelCounter % config.breakEvery === 0) {
      tempGrid.push({ isBreak: true, levelNum: '—', labelText: `Перерыв`, sb: '—', bb: '—', ante: '—', duration: config.breakDuration });
...
const defaultPresets: TournamentPreset[] = [
  { id: 'regular', name: 'Regular', config: { startBB: 4, levelDuration: 15, useAnte: true, anteStartBB: 40, breakEvery: 4, breakDuration: 10, warningTime: 60 } },
  { id: 'turbo', name: 'Turbo', config: { startBB: 4, levelDuration: 8, useAnte: true, anteStartBB: 60, breakEvery: 5, breakDuration: 5, warningTime: 30 } },
  { id: 'hyper', name: 'Hyper', config: { startBB: 10, levelDuration: 3, useAnte: false, anteStartBB: 100, breakEvery: 6, breakDuration: 3, warningTime: 15 } }
];
  ...
  persist(
    (set, get) => ({
      ...
      setConfigValue: (key: any, value: any) => {
        set((state: any) => ({ config: { ...state.config, [key]: value }, activePresetId: 'custom' }));
        get().buildTournament();
      },
      ...
      buildTournament: () => {
        ...
        set({ grid: tempGrid, currentIndex: 0, isPaused: true, secondsLeft: tempGrid.length > 0 ? tempGrid[0].duration * 60 : 0, totalDurationStr: `${h} ч. ${m} мин.` });
      },
      setIsPaused: (paused: boolean) => set({ isPaused: paused }),
      setSecondsLeft: (seconds: any) => set((state: any) => ({ secondsLeft: typeof seconds === 'function' ? seconds(state.secondsLeft) : seconds })),
      nextLevel: () => {
        const { currentIndex, grid } = get();
        const nextIndex = currentIndex + 1;
        if (nextIndex < grid.length) {
          set({ currentIndex: nextIndex, secondsLeft: grid[nextIndex].duration * 60, isPaused: true }); // ЖЕСТКАЯ ПАУЗА ПОСЛЕ КОНЦА ТАЙМЕРА
        } else {
          set({ isPaused: true });
        }
      },
